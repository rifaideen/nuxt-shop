<template>
  <div>
    ...
    <div class="mt-4 mb-4 light-bg-container rounded">
      <div class="p-4">
        <div v-if="errors">
    ...
    <div class="container mb-2" v-if="collections.length > 0">
      <GridItemsRow
...
export default {
  ...
  async asyncData({ store, $axios }) {
    const countries = store.dispatch('getCountries');
    const storeId = store.state.store.id;
    const sliders = $axios.get(`/sliders/${storeId}`);
    const collections = $axios.get('/collections', {
      params: {
        store_id: storeId,
        type: 'event'
      }
    });
    const items = [sliders, collections, countries];
    ...
    return {
      ...
      collections: results[1].data.data,
    }
  },
  data() {
    return {
